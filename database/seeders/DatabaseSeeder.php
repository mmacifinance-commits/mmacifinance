    ...
    public function run(): void
    {
        ...
        foreach ($yearsToSeed as $year) {
            $annualBudget = AnnualBudget::updateOrCreate(
                ['year' => $year],
                [
                    'ref_no' => sprintf('AB-%d-%04d', $year, $year - 2020),
            ...
            for ($month = 1; $month <= 12; $month++) {
                ...
                foreach ($particularData as $parIdx => $p) {
                    ...
                    $monthlyExp = 0;
                    if ($year <= 2025 || ($year === 2026 && $month <= 6)) {
                        $monthlyExp = round($monthlyAppr * (0.60 + ($month % 4) * 0.08), 2);
                    }
                ...
                if ($year <= 2025 || ($year === 2026 && $month <= 6)) {
                    ...
                    foreach ($parIndices as $parKey) {
                        ...
                        $amount = round(($monthlyBaseAllocations[$parKey][$month - 1] ?? 200000) * 0.75, 2);
                        $dateStr = sprintf('%d-%02d-%02d', $year, $month, min(28, 5 + ($parKey * 2)));

                        $isPosted = ($year <= 2025 || $month <= 5);
                        ...
                        $expense = Expense::updateOrCreate(
                            ...
                            [
                                ...
                                'status' => $isPosted ? 'approved' : 'pending',
