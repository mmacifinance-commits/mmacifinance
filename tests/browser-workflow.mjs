...
try {
    ...
    await page.getByRole('button', { name: 'New Annual Budget', exact: true }).click()
    await page.locator('.app-modal input[type="date"]').nth(0).fill('2026-08-01')
    await page.locator('.app-modal input[type="date"]').nth(1).fill('2027-07-31')
    await page.getByRole('button', { name: 'Create Annual Budget', exact: true }).click()
    await page.locator('.app-modal p').filter({ hasText: 'overlaps' }).waitFor()
    assert.equal(await page.locator('[data-global-request-error]').count(), 0)
    assert.equal(await page.getByText(/This period overlaps/).count(), 1)
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    console.log('PASS validation appears once inside the form, without a global duplicate')
